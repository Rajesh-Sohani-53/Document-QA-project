...
while True:
    ...
    # step1: check the radis cache first
    cached_answer =r.get(query)
    if cached_answer:
        print("\n🤖 (from Redis cache):", cached_answer, "\n")
        continue
    #step 2: if it not catched --> run the chain
    result = qa_chain.invoke({"input": query, "chat_history": memory.chat_memory.messages})
    # ✅ Step 3: Store in Redis cache (with 7-day expiry)
    r.setex(query, 604800, result["answer"])  # 604800 seconds = 7 days
    # update the memory
